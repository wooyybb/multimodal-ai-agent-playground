...
Reference Image Parsing은 입력 이미지를 단순 caption으로만 쓰지 않고 identity, appearance, style, composition, colors, identity rules로 구조화하는 단계입니다. 현재는 BLIP caption과 `vision_result`를 rule-based 방식으로 분석하며, 향후 더 강한 VLM을 연결할 수 있는 Visual Context Engineering 계층입니다.

## Identity Parsing

Identity Parsing은 gender, estimated age, species, role, outfit, accessories 같은 캐릭터 보존 단서를 추출합니다. 이 정보는 Character Program에 병합되어 Prompt Compiler가 identity preservation을 더 명확하게 반영할 수 있게 합니다.

## Visual Context Engineering

Visual Context Engineering은 이미지에서 얻은 정보를 prompt 문장으로 바로 변환하지 않고 structured object로 관리하는 설계입니다. Reference Image Parser는 Caption과 Character Program 사이의 중간 표현을 만들어 style transfer와 character preservation의 기반을 제공합니다.

## Standard VLM Schema

Standard VLM Schema는 모든 vision-language provider가 같은 형태의 `vision_result`를 반환하도록 만드는 계약입니다. `caption`, `detailed_description`, `objects`, `character_hints`, `style_hints`, `composition_hints`, `color_hints`, `model`, `provider`, `used_fallback`을 포함합니다.
