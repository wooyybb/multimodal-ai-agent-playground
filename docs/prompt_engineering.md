...
## Sprint36 Prompt Engineering

이번 Sprint는 prompt lifecycle을 저장하고 비교할 수 있게 만드는 데 초점을 두었습니다. user request부터 provider prompt, evaluation prompt, retry prompt까지 기록하면 prompt 개선이 감각이 아니라 증거 기반으로 바뀝니다.
## Sprint39: Context Program Prompt Engineering

이번 Sprint는 단순 prompt 수정이 아니라 Context Engineering Prompt입니다.

Prompt 구조는 다음 의도를 가집니다.

- Task: ContextProgramBuilder 도입
- Architecture: specialist agent 결과를 structured context program으로 통합
- Workspace: project3 내부 작업 제한
- Allowed Files: prompt/context/provider/debug/report 관련 파일만 수정
- Forbidden Files: model tool, memory, UI, API, main workflow 외부 변경 금지
- Requirements: context program 생성, PromptAssembler 참조, ProviderPromptAdapter 참조, Debug Report 기록
- Done Definition: compile, main 실행 가능, report에 context_program 저장

핵심 패턴은 `Context -> Program -> Prompt`입니다. Context Program은 prompt 자체가 아니라 prompt를 생성하기 위한 structured source of truth입니다.
