...
```text
User -> OrchestratorAgent -> VisionAgent -> PromptAgent -> GenerationAgent -> EvaluationAgent -> RetryAgent -> ReflectionAgent
```

현재 버전은 사용자의 이미지 입력과 텍스트 요청을 받아 `OrchestratorAgent`가 전체 workflow를 조율합니다. `VisionAgent`는 이미지를 caption으로 변환하고, `PromptAgent`는 caption과 user prompt를 조합해 final prompt를 생성합니다. 이후 `GenerationAgent`가 final prompt를 받아 PIL 기반 mock image를 생성하고, `EvaluationAgent`가 mock CLIP similarity score를 반환합니다. `RetryAgent`는 score가 threshold보다 낮은지 판단하고, `ReflectionAgent`는 개선 제안과 suggested prompt를 생성합니다.

## Future Structure

```text
User -> OrchestratorAgent -> VisionAgent -> PromptAgent -> GenerationAgent -> EvaluationAgent -> RetryAgent -> ReflectionAgent -> Memory
```

향후 버전에서는 실제 재생성 loop, 더 정교한 개선 제안(Reflection), 기록 저장(Memory)까지 연결할 예정입니다.
